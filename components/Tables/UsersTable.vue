<template>
    <div class="bg-white rounded-lg shadow p-5 m-5 text-gray-700">
        <p class="font-bold mb-5"><i class="fa-solid fa-users"></i> Todos mis usuarios <span
                class="p-1 rounded-lg bg-primary text-white">total {{ this.numberUsers }}</span></p>
        <div class="overflow-x-auto">
            <table class="table w-full">
                ...
                <tbody>
                    <!-- row 1 -->
                    <tr v-for="(user, index) in allusers" :key="user">
                        <td v-if="index >= initLimit && index <= limitPage">
                            {{ index }}
                        </td>
                        <td v-if="index >= initLimit && index <= limitPage">
                            <p class="font-bold">{{ user.name }} <label
                        ...
                        <td v-if="index >= initLimit && index <= limitPage">
                            <div class="flex flex-col text-sm">
                                <p><i class="fa-solid fa-envelope"></i> {{ user.email }} <label
                                        @click="cogerDatos(user.email, `email`, `email`, user.index)" for="eliminaruser"><i
                                            class="fa-solid fa-pen-to-square p-2 text-yellow-600"></i></label></p>
                                <p><i class="fa-solid fa-mobile"></i> {{ user.number }} <label
                                        @click="cogerDatos(user.number, `numero`, `number`, user.index)"
                                        for="eliminaruser"><i
                                            class="fa-solid fa-pen-to-square p-2 text-yellow-600"></i></label></p>
            ...
            <div v-if="allusers === null" class="p-5">
                <p>Ningun usuario encontrado. 🤔</p>
            </div>
        </div>
        <div class="flex flex-col justify-center items-center">
            <div class="btn-group my-5">
                <button class="btn btn-primary" @click="atrasPagina">«</button>
                <p class="btn bg-gray-100 text-gray-700 border-none hover:text-white">Página {{ page }}</p>
                <button class="btn btn-primary" @click="delantePagina">»</button>
            </div>
        </div>
    </div>
</template>
